        ...
        {cards.map((card, index) => (
          ...
          >
            <div className="p-6 flex flex-col items-center justify-center text-center">
              <span className={`p-3 rounded-full bg-green-100 ${index === 0 || index === cards.length - 1 ? 'bg-yellow-100' : ''}`}>
                {card.icon}
              </span>
              ...
              <p className={`mt-2 text-calagreen ${index === 0 || index === cards.length - 1 ? 'text-yellow-600' : ''}`}>{card.info}</p>
            </div>
          </motion.div>
        ))}
      </div>
